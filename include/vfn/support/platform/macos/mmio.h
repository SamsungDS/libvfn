...
static inline leint32_t mmio_read32(void *base_addr, uint64_t offset)
{
	...
	mapping->pci->dev->MemoryRead32(mapping->pci->bar_region_info[mapping->idx].memory_index,
		mapping->offset + offset, &val);
...
static inline leint64_t mmio_lh_read64(void *base_addr, uint64_t offset)
{
	...
	mapping->pci->dev->MemoryRead64(mapping->pci->bar_region_info[mapping->idx].memory_index,
		mapping->offset + offset, &val);
...
static inline void mmio_write32(void *base_addr, uint64_t offset, leint32_t v)
{
	...
	mapping->pci->dev->MemoryWrite32(mapping->pci->bar_region_info[mapping->idx].memory_index,
		mapping->offset + offset, v);
}
...
{
	mmio_write32((uint8_t *)base_addr, offset + 4,
	(leint32_t __force)((uint64_t __force)v >> 32));
	mmio_write32(base_addr, offset, (leint32_t __force)v);
}
